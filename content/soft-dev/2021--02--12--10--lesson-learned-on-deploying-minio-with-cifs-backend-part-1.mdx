...
Apparently, even though the CIFS share is mounted (yay!). MinIO refuses to serve overlay mounts because it wants to control locks of the whole folder. Overlay mounts don't allow that.
...
We're going to revise the architecture a bit in the next part of the articles: [Part 2](2021--02--12--10--lesson-learned-on-deploying-minio-with-cifs-backend-part-2)