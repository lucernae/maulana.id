...
3. I manage these pods using ReplicaSet
...
For requirement 3, It is better to use ReplicaSet because the pod name and PVC name are predictable. If you have a ReplicaSet called `minio`, the 
...
alternatively, we can create each PV manually. We are using ReplicaSet so that new PV are going to be created automatically using the provided StorageClass.

For requirement 4, It is a lot easier to install a MinIO pool using Helm Chart. This is because for each pool you have to create ReplicaSet with defined 
...
For requirement 5, Internal service discovery is a natural feature of Kubernetes. A pod in a replicaset can have internal names like this: 
`minio-0.minio.svc.cluster.local`. If we have service called `minio` that selects pods in a ReplicaSet, we can refer to a specific pod 
...
Since we are going to use ReplicaSet, we hope we can create a StorageClass that can dynamically provision multiple PVC template and assign them 
...
Bitnami chart I mentioned above made it super simple to setup distributed MinIO. They already uses ReplicaSet and customizeable StorageClass.
...
You need to specify `mode: distributed` so that Bitnami chart create scalable ReplicaSet-based deployment with Erasure Code.
...
we expand the capacity by incrementing the zones (analog of adding a new homogenous pool) to `zones: 2`. ReplicaSet will increment the replicas from 
...
## The final architecture diagram of ReplicaSet-based distributed MinIO instance
...
The noticeable difference is that the two server pool (two sets of Storage Boxes) is now managed by one single ReplicaSet.
So actually, one ReplicaSet manages the whole pods from pod 0 to pod 7. Each pod serving one MinIO node from one disk. 
The `minio` command line recognize it as two pools of EC set 4 with the help of Bitnami Helm Charts. Without Bitnami 
Helm chart, we have to create two ReplicaSet, one for each server pool (or EC set).
...
If we were to deploy it manually using ReplicaSet, then we have to delete and recreate the ReplicaSet to adjust the `minio` command
to add the new pool.
...
scattered over 4 PVs. But actually each of the pods is handled by just one ReplicaSet. New PVCs/PVs are created with the parameters from the 
...
By increasing the `zones` parameters, we increase the number of replicase. Since I'm using the same Storage Box, I don't have to change 
...
We can simulate disk downtime by manually downscaling the ReplicaSet. In the initial setup, we used 4 replicas. We can downscale
