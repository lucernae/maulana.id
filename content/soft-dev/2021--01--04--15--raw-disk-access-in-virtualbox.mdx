...
description: Yes. Mounting a physical disk to your VM directly and access it
  from inside your VM.
...
It turns out that this is possible in VirtualBox, but very **highly experimental features**. It may possibly damage your mounted disk in VMs. So, experiment with care. This is an **expert** level features. Only do the following if you understand what you are doing *wink-wink-wink*.
...
Let's use a set of accepted jargon so we have an easier time communicating.
...
4. Create a virtual disk definition using VBoxManage. It will generate a VMDK file in your specified location, with the definition to have raw access to your specified disk device. `filepath` must have `.vmdk` extensions.
   ...
   * As I understand, normally a Mac device uses it's own SSD/brand. So it's very unlikely that you attach your own separate SSD inside your MBP or iMac. In conclusions, your disk probably an external one attached via USB or SD Card. In that case, consider provides a raw disk node instead of the usual disk node. For example use `devicepath` `/dev/rdisk0` instead of `/dev/disk0`.
5. Now that you have your raw disk VMDK file, attach it to your guest OS via the VirtualBox Configuration GUI. It's important that the guest OS uses the same boot mechanism if the mounted disk is a bootable disk. If the raw disk is a UEFI bootabe disk, the guest OS should also use UEFI. Unless you know what you are doing.
...
4. Same command, but with extra parameters: `partitions`
   ...
   b. In Linux, `<partitions>` are comma-separated list of partition numbers you want to expose. For example, if you want to allow access to `/dev/sda1` and `/dev/sda3` then `<partitions>` becomes `1,3`
   ...
   * Additionally, in Linux, you can create specific access so that only required partitions is accessed instead of the whole disk. Append `-relative` parameter in the `VBoxManage` virtual disk creations.
   ...
   c. In MacOS, similar with Linux, `<partitions>` refer to the comma-separated list of partition numbers you want to expose. For example, if you want to allow access to `/dev/disk1` and `/dev/disk3` then `<partitions>` becomes `1,3`
...
Oh, I forgot to mention. You probably need VirtualBox Guest OS Helpers installed in order to seamlessly copy in or out of VM.
