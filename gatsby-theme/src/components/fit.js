...
export const ActivityEmbedViewer = (
  {
    activityId,
    title
  }) => {
  return (
    <div className={"fit-viewer"}>
      <iframe src={`https://connect.garmin.com/modern/activity/embed/${activityId}`} title={title}
              width="465" height="500" frameBorder="0">
      </iframe>
    </div>
  )
}

export const HeartRateViewer = (
  {
    width,
    height,
    useMUI,
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const chartData = records.reduce((acc, cur) => {
