...
class BaseRunnableWidget extends React.Component {
    ...
    findLabelParam(stateName) {
        ...
        if(children.find !== undefined) {
            return children.find(o => o.props.aslabel === stateName)
        }
        // on Astro with client:only directives, children will be a string
        // we need to convert it to React DOM
        const contentChildren = htmr(children.props.value)
        if(contentChildren.find !== undefined) {
            return contentChildren.find(o => o.props.aslabel === stateName)
        }
        return undefined
    }
                                ...
                                Object.entries(this.state.fields).map( ([stateName, stateValue]) => (
                                    <div
                                        ...
                                        <div className="col-span-2 sm:col-span-2">
                                            ...
                                            <input
                                                type="number"
                                                name={stateName} id={stateName} className="focus:ring-indigo-500 focus:border-indigo-500 flex-1 block w-full rounded-none rounded-md sm:text-sm border-gray-300"
