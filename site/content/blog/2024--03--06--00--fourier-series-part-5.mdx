...
As you can see that $d(t)$ is an even symmetric function. So it doesn't matter if it's inverse or forward transform, it is always a pair from a constant.
...
it can be viewed as a singular jump in value using delta function. The value $f_k$ doesn't even has to be purely real (it can be a complex number).

$$
\begin{align*}
\mathcal{F}\left\{ G(f) \right\}(t) &= \mathcal{F}\left\{ a\, \delta(f-f_k) \right\}(t)  \\
&= \int_{-\infty}^\infty a\, \delta(f-f_k) \, e^{-i 2 \pi f t} \, df \\
&= a \, e^{-i 2\pi f_k t} \int_{-\infty}^\infty \, \delta(f-f_k) \, e^{-i 2 \pi (f-f_k) t} \, df \\
&= a \, e^{-i 2\pi f_k t} \, 1 = a \, e^{-i 2\pi f_k t}
\end{align*}
$$
...
$$
\begin{align*}
\mathcal{F}\left\{ \delta(t-t_k) \right\}(f) &= e^{-i 2\pi f t_k} \\
\mathcal{F^2}\left\{ \delta(t-t_k) \right\}(t) &= \mathcal{F}\left\{ e^{-i 2\pi f t_k} \right\}(t)=\delta(t+t_k) \\
\mathcal{F^3}\left\{ \delta(t-t_k) \right\}(f) &= \mathcal{F}\left\{ \delta(t+t_k) \right\}(f)=e^{i 2\pi f t_k} \\
\end{align*}
$$
...
$$
\begin{align*}
\mathcal{F}\left\{ e^{i 2\pi f_k t} \right\}(f) &= \delta(f-f_k) \\
\end{align*}
$$
...
\mathcal{F}\left\{ s(f) \right\}(t) &= \sum_k c_k \, e^{-i 2\pi f_k t} \\
\mathcal{F^2}\left\{ s(f) \right\}(f) &= \mathcal{F}\left\{ \sum_k c_k \, e^{-i 2\pi f_k t} \right\}(f)=\sum_k c_k \delta(f+f_k) = s(-f) \\
\mathcal{F^3}\left\{ s(f) \right\}(t) &= \mathcal{F}\left\{ \sum_k c_k \delta(f+f_k) \right\}(t)= \sum_k c_k \, e^{i 2\pi f_k t} \\
\end{align*}
$$
...
$$
\begin{align*}
\sin(2 \pi f_k t ) &= \frac{e^{i 2 \pi f_k t} - e^{-i 2\pi f_k t}}{2i} \\
\cos(2 \pi f_k t ) &= \frac{e^{i 2 \pi f_k t} + e^{-i 2\pi f_k t}}{2} \\
\end{align*}
$$
...
\begin{align*}
\sin(2 \pi f_k t ) &= \frac{e^{i 2 \pi f_k t} - e^{-i 2\pi f_k t}}{2i} \\
\mathcal{F}\left\{ \sin(2\pi f_k t)\right\}(f) &= \frac{\delta(f-f_k) - \delta(f+f_k)}{2i} \\
\end{align*}
$$
...
\begin{align*}
\cos(2 \pi f_k t ) &= \frac{e^{i 2 \pi f_k t} + e^{-i 2\pi f_k t}}{2} \\
\mathcal{F}\left\{ \cos(2\pi f_k t)\right\}(f) &= \frac{\delta(f-f_k) + \delta(f+f_k)}{2} \\
\end{align*}
$$
...
\mathcal{F}\left\{ r(\frac{t}{T}) \right\}(f) &= \int_{-\infty}^{\infty} r(\frac{t}{T}) \, e^{-i 2\pi ft} \, dt \\
&= \int_{-\frac{T}{2}}^{\frac{T}{2}} 1 \, e^{-i 2\pi f t} \, dt \\
&= \frac{i}{2\pi f} \left( \exp({-i 2\pi f \frac{T}{2}}) - \exp({i 2\pi f \frac{T}{2}})\right) \\
...
The more rigor approach would be to use tempered distribution and Schwartz function to define the transform. However, the building block will be to long for a short article.
...
i 2 \pi f U(f) &= G(f) \\
U(f) &= \frac{-i}{2\pi f} G(f)
\end{align*}
$$

After Fourier transforming left side and right side, we have the equation above.
Suppose that $G(f)$ exists and the function in such a way that it eliminates variable $f$.
So we choose $G(f)=f$.
Rearranging the equation and transforming it back:

$$
\begin{align*}
U(f) &= \frac{-i}{2\pi f} G(f) = \frac{-i}{2\pi f} f \\
&=\frac{-i}{2\pi} \\
\mathcal{F^{-1}}\left\{ U(f)  \right\} &= \mathcal{F^{-1}}\left\{ \frac{-i}{2\pi}  \right\} \\
u(t) &= \frac{-i}{2\pi} \delta(t) \\
\frac{d u(t)}{dt} &= \frac{-i}{2\pi} \delta'(t) \\
g(t) = \frac{-i}{2\pi} \delta'(t) \\
\end{align*}
$$
...
$$
\begin{align*}
\mathcal{F}\left\{ \delta'(t) \right\} &= i 2 \pi f \\
\mathcal{F}\left\{ t \right\} &= \frac{i}{2 \pi} \delta'(f) \\
\end{align*}
$$
...
$$
\begin{align*}
\mathcal{F}\left\{ \delta^{(n)}(t) \right\} &= (i 2 \pi f)^n \\
\mathcal{F}\left\{ t^n \right\} &= \left( \frac{i}{2 \pi} \right)^n \delta^{(n)}(f) \\
\end{align*}
$$
...
$$
\begin{align*}
U(f) &= \frac{-i}{2\pi f} G(f) = \frac{-i}{2\pi f} 1 \\
\end{align*}
$$
...
\begin{align*}
u(t) &= \frac{-i}{2\pi} s(t) \\
\frac{d u(t)}{dt} &= \frac{-i}{2\pi} s'(t) \\
\delta(t) &= \frac{-i}{2\pi} s'(t) \\
s'(t) &= i 2 \pi \delta(t) \\
s(t) &= i 2 \pi \int_{-\infty}^t \delta(t) \, dt + C
\end{align*}
$$
...
$$
\begin{align*}
s(t) &= i 2 \pi (1- \int_{t}^\infty \delta(t) + C \\
s(t) + s(-t) &=i 2 \pi + 2C
\end{align*}
$$
...
We can then understood that $s(t)$ is some kind of step function. The last key is now to realize that $s(t)$ jumps from
