...
&= \int_T C_2 e^{it} dt \\
0 &= \frac{1}{i} C_2 e^{it} |_T = \frac{1}{i} C_2 e^{i(T_{final} - T_{initial})} \\
\end{align*}
$$
...
is if the right hand side uses interval where $e^{i(T_{final} - T_{initial})} = 0$. This interval can be $-\pi$ to $\pi$ or $0$ to $2\pi$.
...
$e^{i(n-k)(T_{final}-T_{initial})} = 0$ for any integer $(n-k)$, except when $n-k=0$.
